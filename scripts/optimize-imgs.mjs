...
const processDir = async dir => {
    ...
    for (const entry of entries) {
        ...
        await img
            .resize(
                needsResize ? { width: MAX_WIDTH, withoutEnlargement: true } : undefined,
            )
        ...
        console.log(
            `${entry.name} -> ${basename(outPath)} (${formatBytes(origSize)} -> ${formatBytes(newSize)}, -${saved}%)`,
        )
...
console.log(
    `Optimizing images in ${ASSETS_DIR}/ -> WebP (quality ${QUALITY}, max ${MAX_WIDTH}px)\n`,
)
